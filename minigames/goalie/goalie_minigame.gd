...
var ball: PackedScene = preload("res://minigames/goalie/scenes/objects/football.tscn")
...
@onready var ball_angle: float = randf_range(-0.5, 0.5)
...
func spawn_ball():
	...
	new_ball.passed_goal.connect(_on_Ball_passed_goal)

	# Rotate the ball.  The angle should be sharper for each ball in sequence.
	var r: float = randf_range(0.0, 0.35)
	if ball_angle < 0.0:
		ball_angle -= r
	else:
		ball_angle += r
	if randi() & 1:
		ball_angle *= -1
	new_ball._velocity = new_ball._velocity.rotated(ball_angle)
