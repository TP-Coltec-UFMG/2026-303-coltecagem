...
@onready var arrow_t = preload( "res://minigames/dance/scenes/ui/dance_arrow.tscn" )
...
func decide():
	...
	if player_score >= SCORE_TO_WIN:
		...
		announcer.add_theme_color_override( "font_color", Color.RED )
	else:
		emit_signal( "lost" )
		announcer.set_text( "Too Bad!" )
		announcer.add_theme_color_override( "font_color", Color.WHITE_SMOKE )


# Spawns an arrow.
func _spawn_arrow():
	var arrow = arrow_t.instantiate()
	# Spawn the arrow.
	add_child( arrow )
	...
	arrow.despawned.connect( _add_score )
...
func _add_score( score: int ):
	...
	if score <= MISS:
		...
		announcer.add_theme_color_override( "font_color", Color.GRAY )
	elif score == OK:
		announcer.set_text( "OK" )
		announcer.add_theme_color_override( "font_color", Color.BLUE )
	elif score == GOOD:
		announcer.set_text( "GOOD!" )
		announcer.add_theme_color_override( "font_color", Color.ORANGE )
	elif score == GREAT:
		announcer.set_text( "GREAT!!" )
		announcer.add_theme_color_override( "font_color", Color.GREEN )
	elif score == PERFECT:
