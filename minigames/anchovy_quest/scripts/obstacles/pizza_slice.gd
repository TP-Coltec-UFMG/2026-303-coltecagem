...
func _init():
	spin_degrees = randf_range( 90.0, 180.0 )
	if randi() % 2 == 0:
		spin_degrees *= -1
...
func _physics_process( delta ):
	...
	if not collision:
		...
		if collision.get_collider().has_signal( "hit" ):
...
func set_slice_velocity( vector: Vector2 ):
	_velocity = vector


# Returns the velocity of the pizza.
func get_slice_velocity() -> Vector2:
	return _velocity


# Sets the speed of the pizza.
func set_speed( speed: float ):
	set_slice_velocity( get_slice_velocity().normalized() * speed )


# Returns the speed of the pizza.
func get_speed() -> float:
	return get_slice_velocity().length()


# Sets the direction of travel.
func set_direction( degrees: float ):
	var vector: Vector2 = get_slice_velocity()
	var new_vector = Vector2( vector.length(), 0 )
	set_slice_velocity( new_vector.rotated( deg_to_rad( degrees ) ) )


# Returns the direction of travel in degrees.
func get_direction() -> float:
	return rad_to_deg( get_slice_velocity().angle() )
