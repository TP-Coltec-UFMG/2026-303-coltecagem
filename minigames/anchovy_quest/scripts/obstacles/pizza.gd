...
func explode():
	var slice1 = _slice.instantiate()
	var slice2 = _slice.instantiate()
	...
	slice1.set_slice_velocity( get_slice_velocity().rotated( deg_to_rad( 45.0 ) ) )
	slice1.set_spin( get_spin() )
	slice2.set_slice_velocity( get_slice_velocity().rotated( deg_to_rad( -45.0 ) ) )
