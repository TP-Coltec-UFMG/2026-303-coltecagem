...
func nanogame_prepare(difficulty: int, _debug_code: int) -> void:
	...
	var sway_states: Array[bool] = _punching_bag.get_sway_states()
	var hits := %Hits as HBoxContainer
	for i: int in sway_states.size():
		if sway_states[i]:
			(hits.get_child(i) as TextureRect).texture =\
					preload("_assets/indicator_sway.png")
...
func _on_boxer_punched() -> void:
	...
	if did_hit:
		(%Hits as HBoxContainer).get_child(_punching_bag.get_hit_count() - 1).\
				texture = preload("_assets/indicator_hit.png")
